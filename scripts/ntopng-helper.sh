...
Examples:
    NTOP_URL=https://ntopng.example.com NTOP_PASS=secret $0 flows
    $0 status
    $0 alerts
...
get_session() {
    local response
    response=$(curl -s -k -i -X POST "$NTOP_URL/authorize.html" \
        -d "user=$NTOP_USER" \
...
query() {
    ...
    curl -s -k -b "session_3000_0=$session" "$NTOP_URL/lua/$endpoint"
}
